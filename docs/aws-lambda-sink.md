...
- Download the NAR package from [the download page](https://github.com/streamnative/pulsar-io-aws-lambda/releases/download/v2.9.2.22/pulsar-io-google-pubsub-2.9.2.22.nar).
...
2. Build the connector in the `pulsar-io-aws-lambda` directory.
     ...
     ```bash
     ls target
     pulsar-io-aws-lambda-2.9.2.22.nar
     ```
  ...
  ```json
    {
      ...
      "archive": "connectors/pulsar-io-aws-lambda-2.9.2.22.nar",
      "parallelism": 1,
      "configs":
      {
...
spec:
  image: streamnative/pulsar-io-aws-lambda:2.9.2.22
  ...
  resources:
    ...
    memory: 1G
  java:
    jar: connectors/pulsar-io-aws-lambda-2.9.2.22.nar
...
--archive pulsar-io-aws-lambda-2.9.2.22.nar \
...
2. Copy the NAR package of the AWS Lambda connector to the Pulsar connectors directory.
    ...
    ```
    cp pulsar-io-aws-lambda-2.9.2.22.nar 
    PULSAR_HOME/connectors/pulsar-io-aws-lambda-2.9.2.22.nar
    ```
